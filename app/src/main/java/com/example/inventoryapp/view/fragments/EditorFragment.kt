...
import com.example.inventoryapp.view.utils.CustomTakePicture
import com.example.inventoryapp.view.utils.loadImage
...
class EditorFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        with(binding){
            addButton.setOnClickListener {
                if (productName.text.isNotEmpty() && price.text.isNotEmpty() && quantity.text.isNotEmpty() && supplier.text.isNotEmpty() && bitmap != null) {
                    ...
                    if (args.product == null){
                        viewModel.insertData(Product(0, name,
                            bitmap!!, price, quantity,false, supplier))
                    }else{
                        val product = args.product!!
                        viewModel.updateData(Product(product.id, name,
                            bitmap!!, price, quantity, product.isArchive, supplier))
                    }
                } else {
                    "Заполни все поля".showToast(requireContext())
                }
            }
        }
        viewModel.dataIsSaved.observe(viewLifecycleOwner) {
            it?.let {
                if (it) {
                    "Сохранено".showToast(requireContext())
                    navigateUp()
                }
            }
        }
  ...
  /*  private fun checkCameraPermissions() {
    ...
    private fun getImage() {
        val uri = getRandomUri(".jpg")
        getContent.launch(uri)
    }

    private fun getRandomUri(
        extension: String, directory: String = Environment.DIRECTORY_PICTURES
    ): Uri {
        val randomFilePath =
            "${requireContext().getExternalFilesDir(directory)?.absolutePath}/${System.currentTimeMillis()}.$extension"

        return getUriFromPath(requireContext(), randomFilePath)
    }
